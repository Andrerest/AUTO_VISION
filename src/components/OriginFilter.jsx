    ...
    <div className="flex items-center gap-1 p-1 bg-card border border-border">
      <button
        ...
        className={`px-4 py-2 font-mono text-xs tracking-[0.15em] transition-colors ${
          selected === "All" ? "bg-foreground text-background" : "text-muted-foreground hover:text-foreground"
        }`}
      >
        TODOS
      </button>
      {origins.map((origin) => (
        <button
          ...
          className={`px-4 py-2 font-mono text-xs tracking-[0.15em] transition-colors ${
            selected === origin ? "bg-foreground text-background" : "text-muted-foreground hover:text-foreground"
          }`}
        >
          {origin.toUpperCase()}
        </button>
